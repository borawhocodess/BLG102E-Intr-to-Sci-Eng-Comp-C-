...
char* get_word(char* s, int n)
{
    ...
    bzero(poss_word, MAX_CHAR_IN_LINE);
...
void remove_punct_and_make_lower_case(char *p)
{
    ...
    while (*not_fixed)
    {
       if (ispunct((unsigned char)*not_fixed))
       {
          not_fixed++;
       }
       else if (isupper((unsigned char)*not_fixed))
       {
          *fixed++ = tolower((unsigned char)*not_fixed);
          not_fixed++;
       }
       else if (not_fixed == fixed)
       {
          not_fixed++;
          fixed++;
       }
